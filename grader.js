...
var urlfn = function(url, checker) {

    rest.get(url).on('complete', function isthere (result, response) {
	if (result instanceof Error) {
	    ...
	    var out = {};
   	    $ = cheerio.load(url_response);
	    var checks = loadChecks(checker).sort();
...
console.log("checks: " + program.checks + "\nresponse: " + url_response + "\nurl: " + program.url);
	...
	console.log("outJason:" + outJson);
	}
    else {
	var outString = urlfn(program.url, program.checks);
